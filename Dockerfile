...
# `--group llm` adds the OpenAI SDK and azure-identity so the runtime reaches Azure OpenAI keyless,
# as the Container App's managed identity. Cosmos access is a base dependency rather than a group,
# because every read the runtime makes goes through it. The heavy ML stack (eval group) stays
# excluded, so the image is lean and downloads no models. Note: no BuildKit cache mount, since
# az acr build uses the classic Docker builder.
...
EXPOSE 8000
# --frozen --no-dev --group llm: run against the locked runtime environment
