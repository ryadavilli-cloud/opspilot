...
| Authored expectations and fixture | `data/answer_key/scenarios.yaml`, `benign_fixture.yaml`, `tests/answer_key.py` | Seven authored records with all required parts, every required reference resolving in the corpus, each now also carrying what an evaluation reads of it: the outcomes the scenario accepts, evidence the corpus deliberately holds none of, the behavior it tests, the alternatives an assessment may reach instead, how retrieved knowledge should matter, the recommendation that fits, and where one applies, the reference whose ordinary reading rules a competing cause out. One expectation shape, in one file; the closure check over generated telemetry covers those references alongside the evidence a correct run must reach. The benign fixture carries only what applies to a non-incident, an accepted outcome and the requirement of an affirmative no-action-now answer, and names which of its four ambient rows it is reported from; it stays structurally invisible to scenario counting. One loader in the tests' own directory reads both files, the builder that used to expose those loaders on its way to a projection having gone with the projection | Nothing |
...
| Offline evaluation | `eval/evaluation.py`, `eval/judge.py`, `eval/prompts/judge.v1.md`, `eval/run_evaluation.py`, `tests/test_evaluation.py`, `tests/test_judge.py` | The deterministic half and the runner around it. Correctness reuses the runtime's own gate rather than restating grounding: references resolve in what the run admitted, `what_happened` and every established candidate rest on admitted evidence, no knowledge reference stands as current proof. Read-only is checked from the operations the run actually attempted against the registry. Deliberately absent evidence must be disclosed, truthfully either way it happened: an authoritative absence in the evidence or a limitation where the source could not answer, and silence satisfies neither. Scenario behavior reads the accepted outcome, and the benign fixture additionally requires an affirmative no-action-now entry rather than an empty one. Each check is proven by mutation, so a wiring that could never fail is caught. One runner replays a scenario that has a recording, obtains the benign fixture live against its own constructed incident context, and reports anything else as not run with the reason named, so coverage is stated rather than implied; a live run that cannot complete is reported the same way instead of ending the report, while a replay that fails is left to raise. One report per run, per-scenario results with named failures, no composite score, recording the configuration identity it ran under, written to a gitignored directory. One offline judge runs after those checks over each record: the runtime's own chat deployment, one authored rubric read from a directory the runtime cannot reach, and a category for each of four qualities of the delivered brief plus the semantic diagnosis match the mechanical layer does not attempt. It reads the brief, the incident as reported, and the expectation, and is shown neither the evidence set nor the parts of the expectation code has already settled, nor the scenario title, which names the cause. A category outside its vocabulary, a missing one, or one given without a reason is refused rather than repaired, and each way a judgement is missing is told apart in the report: the scenario did not run, no deployment is configured, the deployment could not be reached, the call did not complete, the answer was not a judgement. Nothing joins the two: a scenario result has no field a category could be written into, so the report prints them beside each other and the deployment that judged is recorded alongside the one that ran One injection seam on the investigation runner carries the two substitutions a controlled comparison may make, and nothing else: what decides the next action, and whether retrieved passages reach the prompts. It travels with the run dependencies the route constructs itself, so no request reaches it, and it is neither a setting nor persisted state. Absent is the default, so a run given none behaves as one that could not have been given one. Withholding keeps passages out of prompt assembly only: retrieval still runs, still costs its calls, and still reaches the gate and the record, which is what keeps the two conditions comparable. The fixed-path control issues the same capabilities in a predetermined order chosen by how many steps have been taken and by nothing observed, binding its arguments from admitted evidence, and makes no model call, so a run on it accounts for no call it did not make. Adaptive value reports evidence only the adaptive path reached, a red herring only the fixed path rested on, and where the expected cause landed for each condition, that last read from the one judge asked once of each record rather than from anything new. Retrieval influence reports a capability proposed, the leading candidate, an interpretation, or an action, each naming which condition it fell on, and refuses to draw a conclusion where the condition shown its passages retrieved none. Both conditions of either comparison run live, and a condition that cannot be obtained is reported as a comparison nobody could set up rather than ending the report | Nothing |
...
Nothing. Every capability is either whole or absent.

---

## 4. Missing

Nothing. Every capability the governing design requires is present in the repository. What is
left is simplification, which the table above records against the capability it belongs to.
