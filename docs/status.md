...
- **Inspected:** `main` at `fa21547` plus the persistence landing, working tree, 2026-08-18 UTC.
- **Toolchain:** `uv`; Python 3.12.
- **Gates at this tree:** `ruff check` clean; `ruff format --check` clean repository-wide; `mypy`
  strict clean over 83 source files; deterministic lane `pytest -q -m "not llm"`: **717 passed,
...
| Persistence | The completed-investigation model, the `save`/`get` seam, and both backends, as in section 2 | No runtime path writes through it: nothing calls `save`, so the `investigations` container stays declared and empty |
...
**Plural-turn identity.** `turn_id` and `TurnIdentity` in `turn/identity.py`; `turn_id` on spans
and stream events. It has left the evidence set, the assessment, and persistence: what still
carries it is the streaming transport and the tracing correlation attributes.
...
**Infrastructure and packaging.** Replica max 3. In `pyproject.toml`: `azure-cosmos` is now a base
dependency, so every Cosmos read and the completed-investigation store run from the runtime image;
`langgraph-checkpoint-sqlite` (base) and `langchain-azure-cosmosdb` go next, and the `checkpoint`
group is deleted once emptied. In the `Dockerfile`, `--group checkpoint` is dropped from the two
...
Last live-inspected 2026-08-18 UTC, carrying the tree through the retrieval landing: the deployed
...
- **A helper whose only caller is superseded.** `entities_named` in `evidence/references.py` is
  imported by `diagnosis/admission.py` and by nothing else. It is not itself superseded, but it
  should go with its last caller rather than outlive it inside a module nothing else asks that of.
