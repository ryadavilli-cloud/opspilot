...
- **Inspected:** `main` at `0aaf6dd` plus the hardening landing, working tree, 2026-08-19.
...
- **Gates at this tree:** `ruff check` clean; `ruff format --check` clean repository-wide; `mypy`
  ...
  `pytest -q -m "not llm"`: **502 passed, 1 deselected, no xfails**; the pre-commit hook (lint,
  format, em-dash, vocabulary) passes. CI runs one lane.
- **The count rose** by the bounds, execution-outcome, and probe cases the hardening added.
  Readiness had no tests at all before it; it has them now.
...
| Two-axis tool result | `tools/contracts.py`, `tools/errors.py` | Execution outcome and completeness as separate fields with one inline pairing rule that rejects a meaningless combination and content on a non-succeeded outcome; all five outcomes are reachable and each for its own reason, timed out for a read that ran past its bound, unavailable for a source that did not answer, rejected only for a request that did not fit the capability's parameters, failed for a row the capability could not build its record from; no provider message, status, or stack crosses the boundary | Nothing |
...
Application Insights; the three containers are declared. Readiness on that revision reported
four checks ok, one of which embedded a query and searched the index on every probe; this tree
replaces that with two cheap checks and leaves end-to-end proof to the post-deploy smoke run.
The deployment workflow, including its smoke test, passes against that revision.
...
- **Knowledge references resolve against the wrong source at runtime.** Retrieval reads the
  knowledge container and a completed investigation carries the passages it retrieved, while the
  resolver still decides whether a knowledge reference names something real by looking for an
  authored file on disk. The runtime image ships no such files. Nothing is wrong today because
  no run reaches retrieval, and the correction is owned by the steps that make retrieval
  reachable and that reuse the resolver for evaluation. It is recorded here so no one restores
  corpus files to the image to satisfy it.
