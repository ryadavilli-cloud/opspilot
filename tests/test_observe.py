...
def test_metrics_summary_shows_peak_value_and_ref():
    samples = [
        MetricSample(
            service="payment-api", metric="p95_latency_ms", ts=_ts(10, 0), value=50.0, unit="ms"
        ),
        MetricSample(
            service="payment-api", metric="p95_latency_ms", ts=_ts(10, 10), value=820.0, unit="ms"
        ),
    ]
...
def test_logs_summary_collapses_noise_but_keeps_signal_ref():
    logs = [
        LogRecord(
            ...
            service="payment-api",
            level="error",
            message="PaymentGatewayTimeout calling external gateway",
        )
    ]
    logs += [
        LogRecord(
...
def test_summary_is_defensive_on_empty_and_unknown():
    ...
    dep = DeploymentRecord(
        deploy_id="d1", service="checkout-api", ts=_ts(9, 0), version="5.7.2", note="routine"
    )
