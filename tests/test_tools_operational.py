...
def test_query_logs_surfaces_signal_event():
    r = SVC.query_logs(
        service="payment-api",
        start_time=_dt("2026-06-28T09:45:00Z"),
...
def test_query_logs_level_filter():
    ...
    assert r.answered and all(x.level == "error" for x in r.results)


def test_query_logs_unknown_service_empty():
    r = SVC.query_logs(service="nope-api")
    assert r.answered and r.completeness is Completeness.EMPTY and r.results == []


def test_query_logs_invalid_input_error():
    assert SVC.query_logs(service="").outcome is ExecutionOutcome.REJECTED
...
def test_get_metrics_bad_window_error():
    r = SVC.get_metrics(
        service="payment-api",
        start_time=_dt("2026-06-28T11:00:00Z"),
...
def test_dependencies_full_graph():
    ...
    assert r.answered and len(r.results) >= 12
...
def test_dependencies_downstream_and_critical():
    ...
    assert r.answered and all(e.from_service == "checkout-api" for e in r.results)
